...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    try {
            ...
            if(isset($_GET["id"])) $sql = "SELECT `IdRegOvino` as 'id', t2.CateNombre as 'cate', t3.RazaNombres as 'raza',`RegFechadeNacimiento` as 'fn', `RegSexo` as 'sexo', `RegPeso` as 'peso', `RegAntecedentes` as 'ant', t1.`Idcategoria` AS 'idcate', t1.`IdRaza` as 'idraza' FROM `tbregovino` t1, `tbcategoria` t2, `tbraza` t3 WHERE  `IdRegOvino`=TRIM('".$_GET["id"]."') AND t1.Idcategoria=t2.IdCategoria AND t1.IdRaza=t3.IdRaza";
            $stmt = $conn ->prepare($sql);
            if($stmt->execute()){                
    ...
    try {
        $post = json_decode(file_get_contents('php://input'),true);       
        if($post["id_animal"]!="" && $post["fn"]!="" && $post["peso"]!="" && $post["raza"]!="" && $post["sexo"]!="" && $post["cat"]!="" && $post["antecedentes"]!="" && $post["iduser"]!=""){
            
            $bd = new ConfigDb();
            $conn = $bd->conexion();
            $sql = "UPDATE tbregovino SET `RegFechadeNacimiento`=:FN, `RegSexo`=:SEXO , `RegPeso`=:PESO, `RegAntecedentes`=:ANTE, `Idcategoria`=:IDCAT, `IdRaza`=:IDRAZA WHERE `IdRegOvino` =:ID";
            $stmt = $conn ->prepare($sql);
            $stmt->bindParam(":FN",$post["fn"],PDO::PARAM_STR);
            $stmt->bindParam(":SEXO",$post["sexo"],PDO::PARAM_STR);
            $stmt->bindParam(":PESO",$post["peso"],PDO::PARAM_STR);
            ...
            $stmt->bindParam(":IDCAT",$post["cat"],PDO::PARAM_INT);
            $stmt->bindParam(":IDRAZA",$post["raza"],PDO::PARAM_INT);
            $stmt->bindParam(":ID",$post["id_animal"],PDO::PARAM_INT);
            if($stmt->execute()){                
                    header("HTTP/1.1 200 OK");
                    echo json_encode(['code'=>200,'msg' => "OK"]);
